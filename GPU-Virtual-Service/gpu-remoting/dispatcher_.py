...
def sort_gpu_info(gpu_Info_properties,conf):
    if conf == 1:
        ...
        gpu_Info_properties.sort(key=lambda x: (json.loads(x[b'gpu_info'])['Job_num'], -json.loads(x[b'gpu_info'])['memory_free']), reverse=False)
        return gpu_Info_properties
    else:
...
def find_available_gpus(gpuInfoProperties, cnt, priority):
    ...
    if config['DispatcherMethod']['method'] == 1:
        ...
        if priority == 0:
            gpuInfoProperties = sort_gpu_info(gpuInfoProperties, 4)
        for gpuInfoPro in gpuInfoProperties:
            gpu = json.loads(gpuInfoPro[b'gpu_info'])
...
def handle_message(conn, addr, message, gpuInfoProperties):
    if message.startswith("TypeA:"):
        ...
        allocate_gpus,allocate_gpus_properties = find_available_gpus(gpuInfoProperties, gpuCount, priority)
